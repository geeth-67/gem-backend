...
@Repository
public interface GemStoneRepo extends JpaRepository<GemStone , Long> {

    Page<GemStone> findByActiveTrue(Pageable pageable);

    @Query("Select g FROM GemStone g WHERE g.pricePerCarat" +
            " BETWEEN :minPrice AND :maxPrice AND g.caratWeight >= :minCarat AND g.stockQuantity > 0 AND g.active = true")
